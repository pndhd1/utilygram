...
3. Fill out RELEASE_KEY_PASSWORD, RELEASE_KEY_ALIAS, RELEASE_STORE_PASSWORD in gradle.properties to
   access your release.keystore
4. Fill out local.properties (example is provided in local.properties.example):
    - TELEGRAM_APP_ID and TELEGRAM_APP_HASH can be obtained by creating an app
      at https://my.telegram.org/apps
5. Go to https://console.firebase.google.com/, create two android apps with application IDs
   io.github.pndhd1.utilygram and io.github.pndhd1.utilygram.beta,
   turn on firebase messaging and download google-services.json, which should be copied to the same
   folder as TMessagesProj_App.
6. Open the project in the Studio (note that it should be opened, NOT imported).
7. You are ready to compile Utilygram.
